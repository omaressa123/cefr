...
async def transcribe(audio_path: str, condition: bool = True) -> Transcript:
    ...
    """
    if _groq is None and not settings.uses_local_assessor:
        raise RuntimeError("GROQ_API_KEY is not configured and no local STT endpoint set.")
    ...
    async def _call(prompt: Optional[str]):
        ...
        # Local STT via HTTP-compatible Whisper API
        url = f"{settings.local_assessor_url.rstrip('/')}/audio/transcriptions"
        async with httpx.AsyncClient() as client:
            form = httpx.MultipartFormData()
            form.set_field("file", filename, audio_bytes)
            form.set_field("model", settings.stt_model)
            form.set_field("response_format", "verbose_json")
            form.set_field("language", "en")
            form.set_field("temperature", "0.0")
            if prompt:
                form.set_field("prompt", prompt)
