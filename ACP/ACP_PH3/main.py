...
# ===========================================================================
# BRIDGE — Node.js backend → ACP AI pipeline
# ===========================================================================
# These endpoints let the Node.js Express backend delegate turn processing
# to the full ACP pipeline (Whisper STT + partner reply + clause assessor +
# TTS) without the web frontend knowing or caring which path was taken.
#
# Response shape matches what Node.js processTurn() returns so the web client
# needs zero changes.
...
from core.engine import run_turn, generate_report
...
def _turn_result_to_node_shape(result) -> dict:
    """Map ACP TurnResult to the JSON shape the Node.js backend / web expects."""
    errors = []
    assessment = result.assessment or {}
    guided = result.feedback_style == "guided"
    for e in assessment.get("errors", []):
        errors.append({
            "quote": e.get("student_said", ""),
            "category": e.get("tag", "other"),
            "explanation": e.get("explanation", ""),
            "suggestion": e.get("correction", ""),
            "guidedDiscovery": guided,
            "conceptCheckQuestion": (
                f'Look at "{e.get("student_said", "")}" again – what needs to change here?'
                if guided else None
            ),
        })
    audio_b64 = None
    ...
    return {
        ...
        "feedbackStyle": result.feedback_style,
    }


@app.post(f"{API}/bridge/turn-text")
async def bridge_turn_text(body: TextTurnIn):
    """Process a text turn through the full ACP pipeline."""
    try:
        import tempfile, os
        # Write text to a tiny temp audio file using TTS first, then run the
        # full pipeline; or — simpler — bypass STT and go straight to partner.
        from core.engine import generate_partner_reply, assess_turn
        from core.audio import synthesize
        from core.config import resolve_feedback_style

        style = resolve_feedback_style(body.cefr_level, body.feedback_style)
        reply, _ = await generate_partner_reply(
            body.cefr_level, body.topic, [], body.text
        )
        assessment, _ = await assess_turn(
            body.cefr_level, body.topic, body.text, style
        )
        audio_path = await synthesize(reply, body.cefr_level, body.voice_id)
        ...
        log.exception("bridge/turn-text failed")
        raise HTTPException(status_code=500, detail=str(exc))


@app.post(f"{API}/bridge/turn-audio")
async def bridge_turn_audio(
    ...
    """Process an audio turn through the full ACP pipeline (Whisper STT included)."""
    try:
        suffix = "." + (audio.filename or "turn.webm").rsplit(".", 1)[-1]
        ...
        return _turn_result_to_node_shape(result)
    except Exception as exc:
        log.exception("bridge/turn-audio failed")
        raise HTTPException(status_code=500, detail=str(exc))
    finally:
        try:
            import os
...
@app.post(f"{API}/bridge/report")
async def bridge_report(body: ReportIn):
    """Generate a session report using the ACP report model."""
    try:
        report_text, _ = await generate_report(body.cefr_level, body.topic, body.session)
        return {"markdown": report_text}
    except Exception as exc:
        log.exception("bridge/report failed")
        raise HTTPException(status_code=500, detail=str(exc))
