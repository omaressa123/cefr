...
class Settings:
    ...
    assessor_timeout: float = 25.0   # raised: the clause sweep emits more tokens
    report_timeout: float = 45.0
    tts_timeout: float = 10.0
    pronunciation_timeout: float = 12.0

    # --- text to speech ---
    # auto  : Piper when a voice is configured, else edge-tts
    # piper : Piper only. Fails loudly rather than silently going remote.
    # edge  : edge-tts only (the legacy path)
    ...
    # Wav2Vec2 CTC decoded greedily has no language model, so it transcribes
    # what was said rather than what was probable. Disagreement with Whisper is
    # evidence Whisper repaired something. ~360MB, 0.3-0.8x real time on 2 vCPU
    # -- far too slow for the conversational path, fine under DEFER_ASSESSMENT.
    ...
    # The 360MB first download is a one-time cost with nothing to do with
    # decoding a turn. Charging it to acoustic_timeout guaranteed the first
    ...
    # Off by default. Turning it on downloads ~1.2GB of MMS_FA weights and
    # spends 2-6s of CPU per clip on a 2-vCPU box. It buys real acoustic
    # evidence on read-aloud tasks; it does nothing useful for free
    # conversation (see the circularity note in core/pronunciation.py).
    ...
    # to_thread frees the event loop, not the CPU. One CTC forward pass
    # saturates both vCPUs, so unbounded concurrency starves every other
    # learner's turn. Cap it, and cap torch's own thread pool.
    ...
    # --- assessment ---
    # How many of the learner's own recurring categories prime the sweep.
    # Directing a search beats exhorting a model to try harder.
